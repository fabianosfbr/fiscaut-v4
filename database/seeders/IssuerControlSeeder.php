...
class IssuerControlSeeder extends Seeder
{
    ...
    private function criarTiposControle(int $issuerId): void
    {
        $this->command->info('Criando Tipos de Controles...');

        $tipos = [
            [
                'issuer_id' => $issuerId,
                'nome' => 'AVCB',
                'categoria' => IssuerControlTypeEnum::ADMINISTRATIVA->value,
                'descricao' => 'Certificado de Vistoria do Corpo de Bombeiros',
                'alerta_dias_antecedencia' => 15,
                'prioridade' => IssuerControlPriorityEnum::ALTA->value,
            ...
            [
                ...
                'categoria' => IssuerControlTypeEnum::PREVENTIVA->value,
            ...
            [
                ...
                'categoria' => IssuerControlTypeEnum::CORRETIVA->value,
                'descricao' => 'Substituição de lâmpadas queimadas na área de garagem',
                'alerta_dias_antecedencia' => 0,
                'prioridade' => IssuerControlPriorityEnum::BAIXA->value,
            ...
            [
                ...
                'categoria' => IssuerControlTypeEnum::INSPECAO->value,
            ...
            [
                ...
                'prioridade' => IssuerControlPriorityEnum::MEDIA->value,
        ...
        foreach ($tipos as $tipo) {
            IssuerControlType::create($tipo);
        }

        $this->command->info('Tipos de Controles criados com sucesso!');
    }
    ...
    private function criarTemplatesRecorrencia(int $issuerId): void
    {
        ...
        $tipos = IssuerControlType::where('issuer_id', $issuerId)->get();

        $templates = [
            [
                'issuer_id' => $issuerId,
                'type_control_id' => $tipos->where('nome', 'Limpeza de Caixa D\'Água')->first()->id,
                ...
                'frequencia' => IssuerControlFrequencyEnum::MENSAL->value,
                'intervalo' => 1,
                'dia_mes' => 15,
            ...
            [
                'issuer_id' => $issuerId,
                'type_control_id' => $tipos->where('nome', 'Inspeção de Segurança')->first()->id,
                'titulo_template' => 'Inspeção de Segurança - {data}',
                'descricao_template' => 'Inspeção mensal de equipamentos de segurança',
                'frequencia' => IssuerControlFrequencyEnum::SEMANAL->value,
                'intervalo' => 1,
        ...
        foreach ($templates as $template) {
            IssuerControlRecorrency::create($template);
        }

        $this->command->info('Templates de Recorrência criados com sucesso!');
    }

    private function criarControlesIndividuais(int $issuerId): void
    {
        ...
        $controles = [
            ...
            [
                ...
                'status' => IssuerControlStatusEnum::CONCLUIDA->value,
                'prioridade' => 'alta',
            ...
            [
                'issuer_id' => $issuerId,
                'type_control_id' => $tipos->where('nome', 'Troca de Lâmpadas da Garagem')->first()->id,
                'titulo' => 'Troca de Lâmpadas Queimadas - Garagem',
                'descricao' => 'Substituição de lâmpadas queimadas no corredor da garagem',
                'status' => IssuerControlStatusEnum::EM_ANDAMENTO->value,
                'prioridade' => 'baixa',
            ...
            [
                'issuer_id' => $issuerId,
                'type_control_id' => $tipos->where('nome', 'Pintura de Fachada')->first()->id,
                'titulo' => 'Pintura de Fachada - Edifício Principal',
                'descricao' => 'Pintura preventiva da fachada para proteção contra intempéries',
                'status' => IssuerControlStatusEnum::PROGRAMADA->value,
                'prioridade' => 'media',
            ...
            [
                ...
                'status' => IssuerControlStatusEnum::ATRASADA->value,
                'prioridade' => 'alta',
        ...
        foreach ($controles as $controle) {
            IssuerControl::create($controle);
        }

        $this->command->info('Controles Individuais criados com sucesso!');
    }
    ...
    private function criarHistoricoStatus(int $issuerId): void
    {
        ...
        $controles = IssuerControl::where('issuer_id', $issuerId)->get();

        // Histórico para a manutenção concluída (Limpeza de Caixa D'Água)
        $controle1 = $controles->where('titulo', 'Limpeza de Caixa D\'Água - 15/03/2024')->first();
        if ($controle1) {
            $dataBase = now()->subDays(14);

            // Programada → Em Andamento (2 dias depois)
            IssuerControlEventLog::create([
                'issuer_control_id' => $controle1->id,
                'acao' => 'status',
                'status_anterior' => 'programada',
                'status_novo' => 'em_andamento',
                'user_id' => 1, // Supondo que exista um usuário administrador
        ...
        $controle2 = $controles->where('titulo', 'Troca de Lâmpadas Queimadas - Garagem')->first();
        if ($controle2) {
            $dataBase = now()->subDays(12);

            // Programada → Em Andamento
            IssuerControlEventLog::create([
                'issuer_control_id' => $controle2->id,
                'acao' => 'status',
                'status_anterior' => 'programada',
                'status_novo' => 'em_andamento',
                'user_id' => 1,
        ...
        $controle4 = $controles->where('titulo', 'Inspeção de Segurança - 18/03/2024')->first();
        if ($controle4) {
            $dataBase = now()->subDays(16);

            // Programada → Atrasada (quando passou da data programada)
            IssuerControlEventLog::create([
                'issuer_control_id' => $controle4->id,
                'acao' => 'status',
                'status_anterior' => 'programada',
