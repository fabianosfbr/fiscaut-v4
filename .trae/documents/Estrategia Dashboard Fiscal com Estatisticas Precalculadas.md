...
* Pipeline em duas camadas:
  ...
  2. **Jobs agregadores** executam queries por tenant+mês+doc\_tipo/tipo com `GROUP BY` do CNPJ e fazem upsert em `statistic_issuers`.
...
* Criar/ajustar índices compostos nas tabelas fonte para acelerar as agregações mensais:
  ...
  * `nfes`: (tenant\_id, emitente\_cnpj, data\_emissao) e (tenant\_id, destinatario\_cnpj, data\_entrada) + fallback (tenant\_id, destinatario\_cnpj, data\_emissao).

  * `ctes`: (tenant\_id, emitente\_cnpj, data\_emissao) e (tenant\_id, destinatario\_cnpj, data\_entrada) + fallback (tenant\_id, destinatario\_cnpj, data\_emissao).

  * `nfses`: (tenant\_id, tomador\_cnpj, data\_entrada) + fallback (tenant\_id, tomador\_cnpj, data\_emissao) e (tenant\_id, tomador\_cnpj, cancelada).

* Em `statistic_issuers`: índice único acima + índice de leitura (tenant\_id, issuer, data).
...
* Criar um “Fiscal Dashboard” no Filament com Widgets que leem apenas `statistic_issuers`:
  ...
  * **ChartWidget** com série dos últimos 12 meses por doc\_tipo/tipo.
...
* Documentar em `docs/`:
  ...
  * Modelo de dados (statistic\_issuers + índices).
