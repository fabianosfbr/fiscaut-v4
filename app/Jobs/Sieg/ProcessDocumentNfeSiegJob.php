...
class ProcessDocumentNfeSiegJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            Log::channel('sieg_log')->info('NFe da empresa '.$this->issuer->razao_social.' processada com sucesso: '.$this->xml);
    ...
    protected function sendCompletionNotification(): void
    {
        ...
        if ($owner instanceof User) {
            ...
            if ($hasErrors) {
                Notification::make()
                    ->warning()
                    ->title('Importação SIEG NFe concluída com erros')
                ...
                Notification::make()
                    ->success()
                    ->title('Importação SIEG NFe concluída')
                    ->body('Todos os documentos NFe SIEG foram processados com sucesso.')
                    ->actions([
                        Action::make('view')
                            ->label('Ver detalhes')
                            ->button()
