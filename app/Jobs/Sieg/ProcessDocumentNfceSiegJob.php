...
class ProcessDocumentNfceSiegJob implements ShouldQueue
{
    ...
    protected function sendCompletionNotification(): void
    {
        ...
        if ($owner instanceof User) {
            ...
            if ($hasErrors) {
                Notification::make()
                    ->warning()
                    ->title('Importação SIEG NFCe concluída com erros')
                ...
                Notification::make()
                    ->success()
                    ->title('Importação SIEG NFCe concluída')
                    ->body('Todos os documentos NFCe SIEG foram processados com sucesso.')
                    ->actions([
                        Action::make('view')
                            ->label('Ver detalhes')
                            ->button()
