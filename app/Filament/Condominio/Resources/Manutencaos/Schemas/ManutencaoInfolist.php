...
use App\Filament\Infolists\Components\HistoricoManutencaoEntry;
                                        ...
                                        Grid::make(3)
                                            ->schema([
                                                TextEntry::make('status')
                                                    ->label('Status')
                                                    ->badge()
                                                    ->icon(fn(?ManutencaoStatusEnum $state): ?string => $state?->getIcon())
                                                    ->color(fn(?ManutencaoStatusEnum $state): string => $state?->getColor() ?? 'gray')
                                                    ->formatStateUsing(fn(?ManutencaoStatusEnum $state): string => $state?->getLabel() ?? '\u2014'),

                                                TextEntry::make('prioridade')
                                                    ->label('Prioridade')
                                                    ->badge()
                                                    ->icon(fn(?ManutencaoPrioridadeEnum $state): ?string => $state?->getIcon())
                                                    ->color(fn(?ManutencaoPrioridadeEnum $state): string => $state?->getColor() ?? 'gray')
                                                    ->formatStateUsing(fn(?ManutencaoPrioridadeEnum $state): string => $state?->getLabel() ?? '\u2014'),

                                                TextEntry::make('tipo')
                                                    ->label('Tipo')
                                                    ->badge()
                                                    ->icon(fn(?ManutencaoTipoEnum $state): ?string => $state?->getIcon())
                                                    ->color(fn(?ManutencaoTipoEnum $state): string => $state?->getColor() ?? 'gray')
                                                    ->formatStateUsing(fn(?ManutencaoTipoEnum $state): string => $state?->getLabel() ?? '\u2014'),
                                            ]),
                                    ]),
                            ...
                            ->schema([
                                Section::make('Informações básicas')
                                    ->schema([
                                        Grid::make(2)
                                            ->schema([
                                                TextEntry::make('tipoManutencao.nome')
                        ...
                        Tabs\Tab::make('Histórico')
                            ...
                            ->schema([
                                Section::make('Últimas ações')
                                    ->schema([
                                        HistoricoManutencaoEntry::make('historico')
                                            ->hiddenLabel(),
                                    ])
                                    ->collapsible(),
                            ]),
