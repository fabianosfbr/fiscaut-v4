    ...
    <div class="flex items-start justify-between gap-3">
        <div class="min-w-0">
            ...
            @if (filled($record->project))
                <div class="mt-1">
                    <span class="inline-flex items-center rounded-md bg-gray-100 px-2 py-0.5 text-[11px] font-medium text-gray-500 dark:bg-gray-800 dark:text-gray-300">
                        {{ $record->project }}
                    </span>
                </div>
            @endif
        ...
        @if (filled($dueDate))
            <div class="shrink-0 rounded-md bg-primary-50 px-2 py-1 text-xs font-semibold text-primary-600 dark:bg-primary-900/40 dark:text-primary-300">
                {{ $dueDate }}
            </div>
        @endif
    </div>
    ...
    <div class="mt-4 flex items-center justify-between gap-3">
        <div class="flex items-center">
            @forelse ($visibleAssignees as $assignee)
                <div
                    title="{{ $assignee->name }}"
                    class="-ml-2 first:ml-0 flex h-9 w-9 items-center justify-center rounded-full border-2 border-white text-xs font-semibold text-white shadow-sm dark:border-gray-700"
                    style="background-color: {{ $assignee->getAvatarBackgroundColor() }}"
                >
                    {{ $assignee->getAvatarInitials() }}
                </div>
            @empty
                <span class="inline-flex items-center rounded-md bg-gray-100 px-2 py-1 text-xs font-medium text-gray-500 dark:bg-gray-800 dark:text-gray-400">
                    Sem responsáveis
                </span>
            @endforelse

            @if ($remainingAssignees > 0)
                <div class="-ml-2 flex h-9 w-9 items-center justify-center rounded-full border-2 border-white bg-gray-100 text-xs font-semibold text-gray-600 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">
                    +{{ $remainingAssignees }}
                </div>
            @endif
        </div>
