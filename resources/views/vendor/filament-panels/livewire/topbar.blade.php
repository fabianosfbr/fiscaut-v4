<div class="fi-topbar-ctn">
    @php
        ...
        $isCondominioPanel = filament()->getCurrentPanel()->getId() === 'condominio';
    @endphp
                        ...
                        @foreach ($navigation as $group)
                            ...
                            <div @class(['fi-topbar-item', 'fi-active' => $group->isActive()])>
                                <a href="{{ $itemUrl }}"
                                    ...
                                    <x-filament::icon-button :icon="$group->getIcon()" :label="$group->getLabel()" size="xl" />
                ...
                <ul class="fi-topbar-nav-groups">
                    @foreach ($navigation as $group)
                        @php
                            ...
                            if ($isCondominioPanel && !$isGroupActive) {
                                continue;
                            }
                        @endphp
                        ...
                        @if ($groupLabel)
                            @if ($isCondominioPanel)
                                ...
                                @foreach ($group->getItems() as $item)
                                    ...
                                    <x-filament-panels::topbar.item :active="$isItemActive" :active-icon="$itemActiveIcon"
                                        :badge="$itemBadge" :badge-color="$itemBadgeColor" :badge-tooltip="$itemBadgeTooltip" :icon="$itemIcon"
                                        :should-open-url-in-new-tab="$shouldItemOpenUrlInNewTab" :url="$itemUrl">
                                        {{ $item->getLabel() }}
                                    </x-filament-panels::topbar.item>
                                @endforeach
                            @else
                                <x-filament::dropdown placement="bottom-start" teleport :attributes="\Filament\Support\prepare_inherited_attributes(
                                    $groupExtraTopbarAttributeBag,
                                )">
                                    <x-slot name="trigger">
